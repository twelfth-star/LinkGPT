...
class LinkTransformer(nn.Module):
    ...
    def get_pairwise_encoding_from_adj(
        self,
        batch: torch.Tensor, 
        adj_prop=None,
        adj_mask=None,
        ...
        """
        Args:
            batch (torch.Tensor): shape is [2, batch_size]. First row is source nodes and second row is target nodes.
        """
        self.train()
        ...
        return pairwise_feats if not return_weights else (pairwise_feats, att_weights)
    ...
    def get_pairwise_encoding(
        ...
        self.train()
        device = self.device
        train_pos = self.data['train_pos'].clone().to(device)
        ...
        perm = []
        for pair in batch.t().numpy().tolist():
            pair_t = tuple(pair)
            if pair_t in self.data['pair_to_edge_idx'].keys():
                edge_idx = self.data['pair_to_edge_idx'][pair_t]
                perm.append(edge_idx)
            rev_pair = (pair_t[1], pair_t[0])
            if rev_pair in self.data['pair_to_edge_idx'].keys():
                edge_idx = self.data['pair_to_edge_idx'][rev_pair]
                perm.append(edge_idx)
        perm = torch.tensor(list(set(perm)), dtype=torch.long)
        
        adjmask[perm] = 0
        edge2keep = train_pos[adjmask, :]
        masked_adj = SparseTensor.from_edge_index(edge2keep.t(), sparse_sizes=(self.data['num_nodes'], self.data['num_nodes'])).to_device(device)
        ...
        if mask_input:
            ...
            masked_adjt = SparseTensor.from_edge_index(edge2keep.t(), sparse_sizes=(self.data['num_nodes'], self.data['num_nodes'])).to_device(device)
        ...
        return self.get_pairwise_encoding_from_adj(
            batch=batch, adj_prop=masked_adjt, adj_mask=masked_adj, test_set=test_set, return_weights=return_weights
        )
