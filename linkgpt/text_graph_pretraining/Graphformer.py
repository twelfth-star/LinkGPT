# Adapted from Patton (https://github.com/PeterGriffinJin/Patton) and Graphformer (https://github.com/microsoft/Graphormer)
...
class GraphFormers(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        # node_mask: [B, 1, 1, N], additive attention, do not attend bad neighbors
        extended_attention_mask = (1.0 - attention_mask[:, None, None, :]) * -10000.0
        # extended_attention_mask: [B, 1, 1, N], additive attention, do not attend padding tokens
        ...
        # station_placeholder: [B * N, 1, D]
        
        embedding_output = torch.cat([station_placeholder, embedding_output], dim=1)
        # embedding_output: [B * N, L + 1, D]
...
class GraphFormersForLinkPredict(BertPreTrainedModel):
    ...
    def forward(self, center_input, neighbor_input=None, mask=None, **kwargs):
        ...
        if not neighbor_input:
            neighbor_input = center_input.copy() # neighbor_input: [B, L]?
            mask = torch.zeros(center_input['input_ids'].shape[0], 1, dtype=torch.long).to(center_input['input_ids'].device) # mask: [B, 1]?
        ...
        # center_input['input_ids']: [B, L]
        # neighbor_input['input_ids']: [B, N-1, L]
        # input_ids_node_and_neighbors_batch: [B, N, L]
        ...
        # attention_mask_node_and_neighbors_batch: [B, N, L]
